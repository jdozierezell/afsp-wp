...
<a href="http://afsp.org/givingtuesday" target="_blank">
    <div class="modal modal--givingTuesday">
...
<script type="text/javascript">
  ...
  var browserArea = browserWidth * browserHeight;
	var numberOfSnowflakes = browserArea/1500;

</script>
<script src="<?php echo get_template_directory_uri(); ?>/js/fallingsnow_v6.js"></script>
<script type="text/javascript">
	jQuery(document).ready(function($){
		$('.modal__overlay, .modal__close, .modal').attr('style', 'display: block');
		$('body').attr('style', 'overflow: hidden');
		$('.modal__overlay, .modal__close').on('click', function(){
			$('.modal__overlay, .modal__close, .modal, #snowflakeContainer').attr('style', 'display: none');
