...
?>
<div class="gmw-results-wrapper clean <?php echo esc_attr( $gmw['prefix'] ); ?> find-chapter__form-wrapper" data-id="<?php absint( $gmw['ID'] ); ?>" data-prefix="<?php echo esc_attr( $gmw['prefix'] ); ?>">
	<?php if ( $gmw_form->has_locations() ) : ?>
		<div class="gmw-results find-chapter__results-wrapper">
			<?php do_action( 'gmw_search_results_start', $gmw ); ?>
			<div class="find-chapter__results-count">
			...
			<div class="find-chapter__map">
				<?php gmw_results_map( $gmw ); ?>
			</div>
			<?php do_action( 'gmw_search_results_before_loop', $gmw ); ?>
			<ol class="find-chapter__chapters-wrapper">
				<?php
						...
						<div class="wppl-title-holder">
							<h2 class="wppl-h2">
								<a href="<?php echo the_permalink(); ?>">
									<?php echo esc_html( $post->location_count . '. ' . get_the_title() ); ?>
								</a>
								<?php gmw_search_results_distance( $post, $gmw ); ?>
							</h2>
						</div>
						<?php
						do_action( 'gmw_search_result_before_image', $gmw, $post );
						gmw_search_results_featured_image( $post, $gmw );
						do_action( 'gmw_search_result_before_excerpt', $gmw, $post );
						gmw_search_results_post_excerpt( $post, $gmw );
						do_action( 'gmw_search_results_before_taxonomies' , $gmw, $post );
						gmw_search_results_taxonomies( $post, $gmw );
						?>
						<div class="wppl-info">
							<div class="wppl-info-left">
								<?php
								do_action( 'gmw_search_results_before_contact_info', $post, $gmw );
								gmw_search_results_location_meta( $post, $gmw );
								do_action( 'gmw_search_results_before_hours_of_operation', $post, $gmw );
								gmw_search_results_hours_of_operation( $post, $gmw );
								?>
							</div>
	...
	<?php else : ?>
		<div class="gmw-no-results">
			<?php
			do_action( 'gmw_no_results_start', $gmw );
			gmw_no_results_message( $gmw );
			do_action( 'gmw_no_results_end', $gmw );
			?>
		</div>
	<?php endif; ?>
</div>